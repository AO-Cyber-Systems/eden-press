...
- [Objective 07-dart-binding]: Objective 7 WASM-target decision gate RESOLVED at 07-02 execution: STANDARD Go (not TinyGo). The full press battery chain (goldmark GFM + chroma + latex2mathml + bluemonday) compiles clean under standard-Go GOOS=js GOARCH=wasm CGO_ENABLED=0; TinyGo remains a documented future size-optimization spike only.
